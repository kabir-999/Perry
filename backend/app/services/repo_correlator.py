...
def correlate(
    ...
    This increases confidence by proving a vulnerability found in code is accessible on the live site,
    or proving a vulnerability found on the site is caused by vulnerable code.
    """
    ...
    for sf in source_findings:
        # Simple heuristic: if the source file name matches a live path route
        # E.g. file is "routes/api/users.js" and live path is "/api/users"
        file_lower = sf.file.lower()
        matched_path = None
        for path in live_paths:
            ...
            # If the last part of the path is in the filename
            if path_parts[-1] in file_lower:
                matched_path = path
                break
            ...
            if title:
                correlated.append(
                    FindingCandidate(
                        ...
                        confidence="confirmed", # Correlation bumps confidence
                        url=matched_path,
                        evidence=f"File: {sf.file}:{sf.line}\nContext: {sf.code_context}",
                        response_summary=f"Matched endpoint: {matched_path}",
                        description=desc,
                        impact="Confirmed vulnerability via both live endpoint mapping and source code analysis.",
                        remediation="Fix the vulnerable code pattern in the source repository.",
                        dedup_key=f"correlation|{matched_path}|{sf.file}|{sf.line}"
                    )
                )
